...
parser.add_argument("--algo", help='Algorithm to use (rsi|stochrsi)', default='rsi', type=str)
	...
	if ( signal_mode == 'buy' ):
		...
		if ( buy_signal == True ):
			...
			if ( tda_gobot_helper.ismarketopen_US() == True ):
				...
				print('Stock ' + str(stock) + ' not purchased because market is closed')
	...
	# In this mode we will monitor the RSI and initiate a short sale if the RSI is very high
		...
		if ( short_signal == True ):
			...
			if ( tda_gobot_helper.ismarketopen_US() == True ):
				...
				print('Stock ' + str(stock) + ' not shorted because market is closed.')
				if ( args.shortonly == True ):
					exit(1)
				else:
					short_signal == False
